...
| `ALERT_WEBHOOK_URL` + `ALERT_WEBHOOK_SECRET` | необязательный HTTPS webhook и секрет HMAC от 32 байт для SLO-инцидентов |
| `ALERT_SLACK_WEBHOOK_URL` | необязательный Slack Incoming Webhook; используется, если не задан общий webhook |
| `ALERT_TELEGRAM_BOT_TOKEN` + `ALERT_TELEGRAM_CHAT_ID` | необязательный Telegram-канал; используется после webhook и Slack |
...
SLO-события всегда сохраняются во встроенном inbox раздела
«Здоровье платформы». Внешний канал добавляет доставку, но не заменяет
встроенную историю. Тёплый readiness и холодный расчёт имеют раздельные p95-цели.
