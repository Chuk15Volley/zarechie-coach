...
const smokeFetch = (url, options = {}) => fetch(url, { ...options, signal: AbortSignal.timeout(15000) });
const root = await smokeFetch(`${baseUrl}/`, { redirect: 'follow' });
...
const health = await smokeFetch(`${baseUrl}/api/system/health`);
assert.equal(health.status, 401, 'private health endpoint must reject keyless access');
assert.match(health.headers.get('cache-control') || '', /no-store/, 'health data must never be cached');

if (process.env.SMOKE_TRAINER_KEY) {
  const authenticatedHealth = await smokeFetch(`${baseUrl}/api/system/health`, {
  ...
  console.warn('SMOKE_TRAINER_KEY is not set; authenticated infrastructure checks were skipped.');
}

const player = await smokeFetch(`${baseUrl}/api/player/log?token=invalid&date=2026-08-28`);
...
if (process.env.SMOKE_PLAYER_TOKEN) {
  const playerPage = await smokeFetch(`${baseUrl}/player/${encodeURIComponent(process.env.SMOKE_PLAYER_TOKEN)}`);
