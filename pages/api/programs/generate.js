...
const MANUAL_FOCUS_LABELS = {
  camp_ecc_anterior:  'СБОРЫ · ЭКСЦЕНТРИКА · Передняя цепь — ручной выбор тренера; квадрицепс, жим, сгибатель бедра; темп 5-0-X-0; метод не привязан к дню недели',
  camp_ecc_posterior: 'СБОРЫ · ЭКСЦЕНТРИКА · Задняя цепь — ручной выбор тренера; бицепс бедра, ягодицы, верхняя тяга; темп 5-0-X-0; метод не привязан к дню недели',
  camp_ecc_fullbody:  'СБОРЫ · ЭКСЦЕНТРИКА · Всё тело — ручной выбор тренера; интеграция обеих цепей, высокое качество движения; метод не привязан к дню недели',
...
function focusLabelForWorkspace(focus, workspace) {
  const fallback = FOCUS_LABELS[focus] || focus;
  return MANUAL_FOCUS_LABELS[focus]
    || `${fallback} — ручной выбор тренера; день недели не выбирает и не меняет метод`;
}
...
function isIsoFocus(focus) {
  return String(focus || '').startsWith('camp_iso_');
}

function isEccFocus(focus) {
...
Правила методологии одинаковые для Заречья и NK Performance. Для ОБЕИХ команд тренер вручную выбирает метод и тип тренировки:
  • День недели НИКОГДА не выбирает, не запрещает и не заменяет focus/trainingType.
  • Старые подписи «понедельник / вторник / пятница» ниже — только исторические примеры ротации, не правила выбора.
  • Заречье учитывает матчи и перелёты только для коррекции объёма, интенсивности и риска. Календарь не меняет выбранный метод.
  • NK Performance не использует расписание Заречья.
...
Блок D — ПОЗИЦИОННАЯ СПЕЦИФИКА (СТРОГО 2 УПРАЖНЕНИЯ · Слот 1 = мощность/качество · Слот 2 = специфическое игровое движение)
  Принцип метода: передняя цепь = ротационная/передняя мощность | задняя цепь = реактивность/торможение | всё тело = интеграция
...
──────────────────────────────────
МЕТОД — ПЕРЕДНЯЯ ЦЕПЬ (можно выбрать в любой день)
...
Блок D (ПЕРЕДНЯЯ ЦЕПЬ · Слот 1 = ротационная/передняя мощность · Слот 2 = специфическое движение):
...
──────────────────────────────────
МЕТОД — ЗАДНЯЯ ЦЕПЬ (можно выбрать в любой день)
...
Блок D (ЗАДНЯЯ ЦЕПЬ · Слот 1 = реактивность/торможение · Слот 2 = специфическое движение):
  → см. ПОЗИЦИОННАЯ ДИФФЕРЕНЦИАЦИЯ ниже (2 упражнения по позиции)

Блок E: 3 упражнения — см. БЛОК E · ФАЗА 1 (E1 кор динамический + E4 эксцентрика + E2 плечо или E3 голеностоп)

──────────────────────────────────
МЕТОД — ВСЁ ТЕЛО (можно выбрать в любой день)
Интеграция обеих цепей с высоким качеством движения
──────────────────────────────────
...
Блок C (PAP нижнее унилатеральное · вектор которого не было в последних двух сессиях — нижнее восстановилось):
  C1 (тяжёлое, 5-0-X-0): дополняющий вектор — ягодица (SL Hip Thrust) или латеральное движение (Lateral Lunge) — 3-4×5-6/ногу
...
Блок D (ВСЁ ТЕЛО · Слот 1 = наиболее специфичный паттерн · Слот 2 = полный игровой паттерн):
  → см. ПОЗИЦИОННАЯ ДИФФЕРЕНЦИАЦИЯ ниже (2 упражнения по позиции)

Блок E: 3 упражнения — см. БЛОК E · ФАЗА 1 (E1 кор динамический + E4 эксцентрика + E2 плечо или E3 голеностоп · выбирай упражнения, не использованные в последних сессиях)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ФАЗА 2 — ИЗОМЕТРИКА (Недели 4-5 · ручной выбор метода · 65-70 мин)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Блок A: Goblet Squat Hold (угол колена 60-90°, пауза 5 сек в напряжении) → вверх максимально резко | Отдых 2-3 мин
Блок B (PAP-тройка верхнее изо · % и объём → см. БЛОК B ПЕРИОДИЗАЦИЯ · ФАЗА 2): B1 — DB Press изо-пауза 5 сек (локоть 90°) → вверх максимально резко → 90 сек → B3 — тяга (TRX Row / Подтягивания)
Блок C (PAP-тройка нижнее унилатеральное изо — нижнее восстановилось): C1 — изо-пауза 5 сек колено (Bulgarian Split Squat изо 90° | Wall Sit одна нога | Step-Down медленный) → вверх максимально резко → 90 сек → C3 — вспомогательное ягодица
...
──────────────────────────────────
ЗАДНЯЯ ЦЕПЬ — ИЗОМЕТРИКА (ручной выбор, любой день)
──────────────────────────────────

Блок A: SL RDL пауза 45° или KB Hinge Hold (задняя цепь, пауза 5 сек в напряжении) → вверх максимально резко | Отдых 2-3 мин
Блок B (PAP-тройка верхнее изо · % и объём → см. БЛОК B ПЕРИОДИЗАЦИЯ · ФАЗА 2): B1 — DB Press изо-пауза 5 сек (локоть 90°) → вверх максимально резко → 90 сек → B3 — тяга (TRX Row / Подтягивания обратным хватом)
Блок C (PAP-тройка нижнее унилатеральное изо задняя цепь — нижнее восстановилось): C1 — изо-задняя цепь (SL Hip Thrust пауза наверху | SL Glute Bridge пауза 5 сек) → вверх максимально резко → 90 сек → C3 — вспомогательное (Slider Hamstring Curl | Kickstand RDL лёгкий)
Блок D: позиционное (см. ниже)
Блок E: 3 упражнения — см. БЛОК E · ФАЗА 2 (E1 кор изометрика + E4 ISO удержание + E2 плечо или E3 голеностоп)

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
ФАЗА 3 — ВЗРЫВ / ПОТЕНЦИАЦИЯ (Неделя 6 · ручной выбор метода · 60 мин СТРОГО · ТЕЙПЕР)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Структура по выбору тренера: передняя цепь | задняя цепь | интеграция (та же логика что фаза 1)
...
НАПАДАЮЩИЕ — ДОИГРОВЩИКИ / ДИАГОНАЛЬНЫЕ (OH / OPP):
  ...
  Блок D выбирай по ручному методу: передняя цепь = ротация/разбег; задняя = COD/торможение; всё тело = интеграция

  ПЕРЕДНЯЯ ЦЕПЬ (ротация + разбег):
  ...
  ВСЁ ТЕЛО (интеграция — наиболее специфичный метод):
...
ЦЕНТРАЛЬНЫЕ БЛОКИРУЮЩИЕ (MB):
  ...
  ПЕРЕДНЯЯ ЦЕПЬ (боковая мощность + атака с 1 шага):
  ...
  ЗАДНЯЯ ЦЕПЬ (боковое перемещение + скрестный шаг):
...
ЛИБЕРО (Libero):
  ...
  БЛОК D (специфика защитной работы · не меняется по фазам):
    ...
    ВСЁ ТЕЛО (наиболее реактивный метод):
...
ПОЛЯ cue, autoReg — русский язык, профессиональный S&C, без воды:
  ...
  ✅ Изометрическая фаза: "Темп: опускайся вниз под контролем; пауза внизу — 5 секунд; поднимайся вверх максимально резко. Таз нейтрален."
...
export async function buildGenerationInputs(body) {
  const { playerId, date, dayGoal = '', days = 7, focus = 'inseason', trainingType = '', powerMode = 'auto', notes = '', warmupSummary = '', teamUsedExercises = [], coachRecovery = 'green', workspace = 'zarechie', planningMode = '', microcycleSlot = null } = body || {};
  ...
  const automaticDoseRecovery = strictestRecoveryStatus(
    matchDayAutomaticRecoveryStatus(readiness.decision, seasonDecision?.key === 'match_day'),
    powerKpiRecovery,
  );
  ...
  if (effectiveRecovery !== coachRecovery || powerContext?.mode === 'recovery') {
  ...
  try {
    ...
    if (workspace === 'zarechie' && events.length > 0) {
      ...
      if (daysToNext) {
        ...
        if (daysSinceLast === 1) {
          lines.push('→ КОРРЕКЦИЯ НАГРУЗКИ: день после игры — СОХРАНИ выбранный тренером метод, но работай консервативно, JLU=0, без тяжёлой силы и осевой нагрузки');
        } else if (daysToNext === 1 || (daysToNext === 2 && hasTravelSoon)) {
          lines.push('→ КОРРЕКЦИЯ НАГРУЗКИ: 1-2 дня до игры / перелёт — СОХРАНИ выбранный метод, сократи объём до 30-40 мин, JLU ≤80, без накопительной усталости');
        } else {
          lines.push(`→ КОРРЕКЦИЯ НАГРУЗКИ: ${daysToNext} дн. до игры — выбранный метод выполняется с полной нагрузкой по готовности, JLU по лимиту фазы`);
        }
      } else {
        lines.push('• Ближайших игр в календаре нет (ближайшие 3 недели)');
        if (daysSinceLast === 1) {
          lines.push('→ КОРРЕКЦИЯ НАГРУЗКИ: день после игры — СОХРАНИ выбранный метод, но снизь объём консервативно, даже если игровая нагрузка ещё не отмечена');
        }
      }
      scheduleContext = '\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n' + lines.join('\n') + '\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n';
    }
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
${scheduleContext}
Метод подготовки (ручной выбор тренера): ${focusLabel}
