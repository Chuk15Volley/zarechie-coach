...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    const raw = await redis('get', `coach:restrictions:${playerId}`).catch(() => null);
  ...
  if (req.method === 'POST') {
    const { playerId, restrictions } = req.body || {};
    if (!playerId) return res.status(400).json({ error: 'playerId required' });
    const clean = Array.isArray(restrictions) ? restrictions.filter(r => VALID.has(r)) : [];
    await redis('set', `coach:restrictions:${playerId}`, JSON.stringify(clean));
