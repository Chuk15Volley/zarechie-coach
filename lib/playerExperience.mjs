import { actualTarget, repetitionInput } from './playerGym.mjs';
export const SKIP_REASONS = ['Занят тренажёр', 'Нет оборудования', 'Дискомфорт', 'Решение тренера'];
    ...
    for (const [bi, block] of (record.session?.blocks || []).entries()) {
      for (const [ei, ex] of (block.exercises || []).entries()) {
        ...
        const sets = record.log ? (ex.targetSets || []).flatMap((target, si) => {
          ...
          return record.log.done?.[setKey] ? [{ target: actualTarget(target, record.log.repetitions?.[setKey]), kg: Number(String(record.log.weights?.[setKey] || '').replace(',', '.')) || null }] : [];
        }) : (actual?.setActuals || []).filter(set => set.completed).map(set => ({ target: actualTarget(set.target, repetitionInput(set.target)?.perSide ? set.reps / 2 : set.reps), kg: set.kg || null }));
