...
function playerFromRecord(p, id) {
  return {
    ...
    position: p.position || '',
    whoopUserId: p.whoopUserId || p.whoop_id || String(id),
  };
}
...
async function getPlayerInfo(id, workspace = 'zarechie') {
  ...
  if (workspace === 'nkperf') {
    const nkRosterRaw = await redis('get', 'nkperf:roster').catch(() => null);
    const found = rosterMatch(parseJSON(nkRosterRaw), id);
    if (found) return playerFromRecord(found, id);
    // fall through to shared keys just in case
  }
