...
import { getReadySixPlayerContext, getReadySixRoster, usesReadySix } from './readySixClient';
import { normalizeReadySixPlayerSnapshot, normalizeReadySixRoster } from './readySixSnapshotAdapter';
...
export async function getPlayerInfo(id, workspace = 'zarechie') {
  if (usesReadySix(workspace)) {
    const payload = await getReadySixRoster(workspace, todayISO());
    const player = normalizeReadySixRoster(payload).find(item =>
      String(item.id) === String(id) || String(item.readySixPlayerId) === String(id)
    );
    return player ? {
      id: String(player.id),
      readySixPlayerId: String(player.readySixPlayerId),
      name: player.name,
      position: player.position,
      whoopUserId: player.whoopUserId || player.id,
      photo: null,
    } : null;
  }
...
function sessionSurveyKey(id, date, period) {
  return `survey:session:${id}:${date}:${period}`;
}

function sessionSurveyDatesKey(id, period) {
  return `survey:session:dates:${id}:${period}`;
}

function latestSessionSurveyKey(id, period) {
  return `survey:session:latest:${id}:${period}`;
}

function preferNewestRecord(current, candidate) {
  if (!candidate) return current || null;
  if (!current) return candidate;
  const byDate = String(candidate.date || '').localeCompare(String(current.date || ''));
  if (byDate > 0) return candidate;
  if (byDate < 0) return current;
  return String(candidate.submittedAt || '') >= String(current.submittedAt || '') ? candidate : current;
}
...
// Post-morning is a separate questionnaire in the dashboard. It contains the
// actual morning gym/volleyball load, sRPE, fatigue, participation and pain.
// Historically the coach generator only read morning wellness + evening data,
// silently omitting this third source.
async function getSessionSurveyHistory(id, period, days, targetDate, workspace = 'zarechie', sourceId = id) {
  if (workspace !== 'zarechie') return [];
  const { id: rid, dates: known } = await resolveDatedId(v => sessionSurveyDatesKey(v, period), id, sourceId);
  const cutoff = daysBefore(targetDate, days - 1);
  const recent = known.filter(date => date >= cutoff && date <= targetDate).sort();
  const indexed = await getDatedBatch(recent, date => sessionSurveyKey(rid, date, period));

  let latest = null;
  for (const variant of sourceIdVariants(id, sourceId)) {
    const record = parseJSON(await zarechieRead('get', latestSessionSurveyKey(variant, period)).catch(() => null));
    const date = dateOf(record);
    if (!record || !date || date < cutoff || date > targetDate) continue;
    latest = preferNewestRecord(latest, { date, ...record });
  }
  if (!latest) return indexed;

  const byDate = new Map(indexed.map(record => [record.date, record]));
  byDate.set(latest.date, preferNewestRecord(byDate.get(latest.date), latest));
  return [...byDate.values()].sort((a, b) => a.date.localeCompare(b.date));
}

async function getLatestSessionSurvey(id, period, targetDate, workspace = 'zarechie', sourceId = id) {
  if (workspace !== 'zarechie') return null;
  const { id: rid, dates } = await resolveDatedId(v => sessionSurveyDatesKey(v, period), id, sourceId);
  const latestDate = [...dates].filter(date => date <= targetDate).sort().pop();
  let latest = latestDate
    ? (await getDatedBatch([latestDate], date => sessionSurveyKey(rid, date, period)))[0] || null
    : null;

  // The latest pointer is written in the same dashboard pipeline as the dated
  // record. Reading both protects generation from a briefly stale date index.
  for (const variant of sourceIdVariants(id, sourceId)) {
    const record = parseJSON(await zarechieRead('get', latestSessionSurveyKey(variant, period)).catch(() => null));
    const date = dateOf(record);
    if (!record || !date || date > targetDate) continue;
...
async function getNeuroHistory(id, workspace = 'zarechie', sourceId = id) {
  ...
  let latest = null;
  if (snapshotRaw) {
    const db = parseJSON(snapshotRaw);
  ...
  for (const raw of histRaws) {
    ...
    if (Array.isArray(h) && h.length) { history = h; break; }
  }
...
export async function getPlayerSnapshot(id, days = 7, targetDate = todayISO(), chronicDays = 28, workspace = 'zarechie') {
  if (usesReadySix(workspace)) {
    const historyDays = Math.max(Number(days) || 7, Number(chronicDays) || 28);
    const payload = await getReadySixPlayerContext(workspace, id, targetDate, historyDays);
    return normalizeReadySixPlayerSnapshot(payload, { days, chronicDays, targetDate });
  }
  ...
  const [whoop, surveys, morning, postMorningSurveys, neuro, manual, chronicWhoop, chronicSurveys, chronicPostMorningSurveys, injuryLog, annotations, latestSurvey, latestMorning, latestPostMorning] = await Promise.all([
    ...
    getSessionSurveyHistory(id, 'morning', days, targetDate, workspace, sourceId),
    ...
    chronicDays > days ? getSessionSurveyHistory(id, 'morning', chronicDays, targetDate, workspace, sourceId) : Promise.resolve(null),
    ...
    getLatestSessionSurvey(id, 'morning', targetDate, workspace, sourceId),
  ]);
  ...
  return {
    ...
    chronicPostMorningSurveys: chronicPostMorningSurveys ?? postMorningSurveys,
