...
export function resolveSeasonSession({
  ...
  if (calendar.todayEvent?.type === 'game') {
    apply(SEASON_SESSION_TYPES.primer, 'match_day', 'День матча: допустим только короткий нейромышечный primer.');
...
export function formatSeasonDecisionForPrompt(decision) {
  ...
  const lines = [
    ...
    `• Предыдущий матч: ${c.previousGame || '—'}${c.daysSinceGame != null ? ` (MD+${c.daysSinceGame})` : ''}; следующий: ${c.nextGame || '—'}${c.daysToGame != null ? ` (MD-${c.daysToGame})` : ''}.`,
    `• Игровая нагрузка после предыдущего матча: ${decision.previousMatchStatus}.`,
