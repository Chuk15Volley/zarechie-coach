...
export function exerciseId(exercise) {
  const existing = String(exercise?.exerciseId || exercise?.canonicalId || '').trim();
  return existing || canonicalExerciseId(exercise?.name || exercise?.title || 'exercise');
}

export function ensureSessionExerciseIds(session) {
