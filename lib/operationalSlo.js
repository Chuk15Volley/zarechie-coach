...
import { readinessLatencyKey, summarizeReadinessLatency } from './readinessTelemetry.js';
...
export async function evaluateOperationalSlo(workspace, options = {}) {
  ...
  const [latencyRows, eventRows] = await pipeline([
  ...
  const readiness = summarizeReadinessLatency(latencyRows, { now });
