...
export async function evaluateOperationalSlo(workspace, options = {}) {
  ...
  const segmentedSamples = Number(readiness.warm?.sampleCount || 0) + Number(readiness.cold?.sampleCount || 0);
  const legacyP95Active = segmentedSamples === 0 && readiness.enoughSamples && !readiness.healthy;
