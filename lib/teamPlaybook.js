...
import { exerciseTonnage, weightKgFromExercise } from './tonnage';
...
function computeTonnage(session) {
  ...
  for (const block of session.blocks || []) {
    for (const ex of block.exercises || []) {
      const weight = weightKgFromExercise(ex);
      if (!weight) continue;
      const targetSets = Array.isArray(ex.targetSets) ? ex.targetSets : [];
      const totalReps = targetSets.reduce((total, set) => {
        const multiple = String(set || '').match(/^(\d+)\s*[x×]\s*(\d+)$/i);
        return total + (multiple ? parseInt(multiple[1], 10) * parseInt(multiple[2], 10) : parseInt(set, 10) || 0);
      }, 0);
      if (!totalReps) continue;
      total += exerciseTonnage(ex, totalReps, weight);
    }
  }
  return total;
}
