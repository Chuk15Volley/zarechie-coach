import { combineReadySixPlanningContext, readySixToday } from './readySixPlanning.mjs';
...
// ReadySix accepts canonical and source identities, but returns a stable
// integration id. Compare only explicit identities from the org-validated player.
function matchesReadySixPlayer(player, requestedId) {
  const identity = value => (typeof value === 'string' || typeof value === 'number') ? String(value).trim() : '';
  const wanted = identity(requestedId);
  const whoop = identity(player?.identities?.whoopUserId);
  const aliases = [player?.id, player?.readySixPlayerId,
    player?.identities?.canonicalPlayerId, player?.identities?.sourceId,
    whoop, whoop ? `whoop_${whoop}` : null].map(identity).filter(Boolean);
  return Boolean(wanted && aliases.includes(wanted));
}
...
async function readySixFetch(workspace, params, expectedMode, options = {}) {
  ...
  try {
    ...
    if (expectedMode === 'player-context' && !matchesReadySixPlayer(validated.player, params.playerId)) {
...
async function planningFetch(workspace, params, mode, options) {
  const today = readySixToday(options.now);
  if (params.date <= today) return readySixFetch(workspace, params, mode, options);
  const [target, current] = await Promise.all([
    readySixFetch(workspace, params, mode, options),
    readySixFetch(workspace, { ...params, date: today }, mode, options),
  ]);
  return combineReadySixPlanningContext(target, current);
}

export function getReadySixPlayerContext(workspace, playerId, date, historyDays = 28, options = {}) {
  return planningFetch(workspace, { playerId, date, historyDays }, 'player-context', options);
}

export function getReadySixTeamReadiness(workspace, date, options = {}) {
  return planningFetch(workspace, { date, view: 'readiness' }, 'team-readiness', options);
}
