...
export function recommendNextLoad({
  ...
  if (readiness.readySixState) {
    const state = readiness.readySixState;
    const hold = state.needsReview;
    const cap = hold ? 0 : finite(state.capPercent, 0);
    const volumePercent = Math.min(cap, rtp.active ? rtp.status === 'paused' ? 0 : rtp.phase.volumeCap : 100);
    return {
      mode: hold ? 'hold' : rtp.active ? 'rtp' : cap < 100 ? 'reduce' : 'maintain',
      label: hold ? 'Решение штаба в ReadySix' : state.label,
      volumePercent, intensityPercent: hold ? 0 : 100, rpeCap: hold ? 0 : rtp.active ? rtp.phase.rpeCap : 8,
      confidence: ['high', 'medium'].includes(state.confidence) ? state.confidence : 'low',
      reasons: [state.detail || state.label],
      safeguards: ['Состояние рассчитано в ReadySix. Здесь применяется только к программе зала.'],
...
export function buildTodayDecisionCenter(rows = [], readinessPlayers = [], platformHealth = null) {
  ...
  const athletes = safeArray(rows).map(row => {
    ...
    const recommendation = (!ready.readySixState && status.adaptation) || recommendNextLoad({
      readiness: ready,
      status,
    ...
    if ((status.live?.completed || status.live?.endedEarly) && !status.feedback) priority = Math.max(priority, 55);
  ...
  const summary = athletes.reduce((acc, item) => {
    ...
    if (item.status.live?.completed || item.status.live?.endedEarly) acc.completed += 1;
