...
export function normalizeReadySixPlayerSnapshot(payload, { days = 7, chronicDays = 28, targetDate } = {}) {
  ...
  const observationDate = payload.planning?.assessmentDate || resolvedDate;
  const historyWindow = Math.max(days, chronicDays);
  const whoopAll = normalizeDated(monitoring.whoop, observationDate, historyWindow);
  const morningAll = normalizeDated(monitoring.morning, observationDate, historyWindow);
  const eveningAll = normalizeDated(monitoring.evening, observationDate, historyWindow);
  const postEveningAll = normalizeDated(monitoring.postEvening, observationDate, historyWindow);
  const postMorningAll = normalizeDated(monitoring.postMorning, observationDate, historyWindow);
  ...
  const neuro = monitoring.neuro
    ...
    : null;
  const manual = Object.fromEntries(Object.entries(monitoring.manual || {}).filter(([date]) => date <= observationDate));
  for (const row of monitoring.vertHistory || []) {
    if (!row?.date || row.date > observationDate || row.jumps == null) continue;
    manual[row.date] = { ...(manual[row.date] || {}), jumps: Number(row.jumps), source: 'vert_team_pdf' };
  }
  if (monitoring.vert?.jumps != null && observationDate) {
    manual[observationDate] = { ...(manual[observationDate] || {}), jumps: Number(monitoring.vert.jumps), source: 'vert_team_pdf' };
  }

  const overtraqHistory = normalizeDated(monitoring.overtraqHistory, observationDate, historyWindow);
  for (const row of overtraqHistory) {
    if (row.jumps != null) manual[row.date] = { ...(manual[row.date] || {}), jumps: Number(row.jumps), source: 'overtraq_team_pdf' };
  }
  if (monitoring.overtraq?.jumps != null && (payload.planning?.assessmentDate || payload.date) === observationDate) {
    manual[observationDate] = { ...(manual[observationDate] || {}), jumps: Number(monitoring.overtraq.jumps), source: 'overtraq_team_pdf' };
  }
  return {
    overtraq: monitoring.overtraq || null,
    overtraqHistory,
    readySixCalendar: payload.calendar || null,
    player: {
    ...
    readySixPlanning: payload.planning || null,
    readySixMeta: {
