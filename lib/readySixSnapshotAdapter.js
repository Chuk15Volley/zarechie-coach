...
export function normalizeReadySixPlayerSnapshot(payload, { days = 7, chronicDays = 28, targetDate } = {}) {
  ...
  const overtraqHistory = normalizeDated(monitoring.overtraqHistory, resolvedDate, historyWindow);
  ...
  if (monitoring.overtraq?.jumps != null && payload.date === resolvedDate) {
    manual[resolvedDate] = { ...(manual[resolvedDate] || {}), jumps: Number(monitoring.overtraq.jumps), source: 'overtraq_team_pdf' };
  }
  return {
