...
export async function getNKWhoopHistory(whoopId, days = 28) {
  const d = await nkFetch(`/api/whoop/history?whoopId=${encodeURIComponent(whoopId)}&days=${Math.min(90, Math.max(1, Number(days) || 28))}`);
...
export async function getNKMorningSurvey(whoopId, date) {
  const d = await nkFetch(`/api/survey/morning-get?playerId=${encodeURIComponent(whoopId)}&date=${encodeURIComponent(date)}`);
  return d?.survey || d || null;
}

export async function getNKSurveyHistory(whoopId) {
  const d = await nkFetch(`/api/survey/history?whoopId=${encodeURIComponent(whoopId)}`);
  return Array.isArray(d?.data) ? d.data : [];
}
