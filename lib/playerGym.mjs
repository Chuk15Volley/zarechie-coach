...
// Planned load is a suggestion; completion requires an explicitly entered actual.
export function validActualWeight(value) {
 if (!['string', 'number'].includes(typeof value) || String(value).trim() === '') return null;
 const text = String(value).trim().replace(',', '.');
 if (!/^\d{1,3}(?:\.\d{1,2})?$/.test(text)) return null;
 const weight = Number(text);
 return Number.isFinite(weight) && weight >= 0 && weight < 1000 ? weight : null;
}
export function setCompletionIssue(ex, target, weight, repetitions) {
 if (needsLoadEntry(ex) && validActualWeight(weight) == null) return 'Укажи фактический вес или подтверди вес по плану.';
 if (repetitionInput(target) && repetitions != null && validRepetitions(repetitions) == null) return 'Укажи фактическое количество повторений.';
 return '';
}

export function weightChangeSummary(session, done = {}, weights = {}) {
 return (session?.blocks || []).flatMap((block, bi) => (block.exercises || []).flatMap((ex, ei) => {
  const planned = Number(ex.weightKg) || Number(String(ex.weightNote || '').match(/(\d+(?:[.,]\d+)?)\s*(?:кг|kg)/i)?.[1]?.replace(',', '.'));
  if (!(planned > 0)) return [];
  const actual = (ex.targetSets || []).flatMap((_, si) => {
   const key = `${bi}-${ei}-${si}`, kg = validActualWeight(weights[key]);
   return done[key] && kg != null && kg !== planned ? [kg] : [];
  });
  return actual.length ? [{ code: ex.code, name: playerExerciseName(ex), planned, actual: [...new Set(actual)], count: actual.length }] : [];
 }));
}
