...
import { readySixToday } from './readySixPlanning.mjs';
...
export function readySixGymState(decision) {
  ...
  return { source: 'ReadySix', code: `readysix_${recommendation}`, level,
    ...
    dataInsufficient: recommendation === 'insufficient_data',
...
export function readySixCalendarContext(calendar, targetDate) {
  ...
  const nearbyConflicts = valid ? (calendar.conflicts || []).filter(event => Math.abs((Date.parse(event.date) - Date.parse(targetDate)) / 86400000) <= 3) : [];
  const upstreamResolved = conflict => events.some(event => event.date === conflict.date && event.source === 'reports'
    && event.type === (conflict.reportType === 'match' ? 'game' : conflict.reportType === 'flight' ? 'travel' : conflict.reportType));
  return { ...seasonCalendarContext(events, targetDate), available: !!calendar && calendar.date === targetDate,
    ...
    conflicts: nearbyConflicts.filter(conflict => !upstreamResolved(conflict)),
    resolvedConflicts: nearbyConflicts.filter(upstreamResolved), events };
}

export function recommendGymSession({ snapshot, targetDate, recentSessions = [], today = readySixToday() } = {}) {
  ...
  const planning = snapshot?.readySixPlanning || (state.dataInsufficient ? {
    preliminary: true, targetDate, assessmentDate: targetDate, requiresDayOfReview: true,
    latestObservationDate: [snapshot?.latestMorning?.date, snapshot?.latestSurvey?.date, ...(snapshot?.whoop || []).map(row => row.date)].filter(date => date && date <= targetDate).sort().at(-1) || null,
  } : null);
  const reasons = [];
  const warnings = calendar.resolvedConflicts.length ? ['В дневном плане есть расхождение; применяется основной календарь, выбранный ReadySix.'] : [];
  if (planning?.preliminary) warnings.push(`Предварительная программа на ${targetDate}. Аналитика ReadySix на ${planning.assessmentDate}; последние наблюдения: ${planning.latestObservationDate || 'не переданы'}. Перед тренировкой проверьте новые данные и при необходимости скорректируйте программу.`);
  if (state.dataInsufficient) warnings.push('Анкет пока недостаточно. Это не запрет ReadySix: можно подготовить программу по доступным данным без подтверждения готовности на день тренировки.');
  ...
  const base = { source: 'ReadySix', targetDate, state, calendar, planning,
  ...
  if (state.hardStop || (state.capPercent != null && state.capPercent <= 20) || (state.needsReview && !state.dataInsufficient)) {
  ...
  if (['rest', 'travel', 'volleyball_only'].includes(plan)) {
    ...
    if (planning?.preliminary && targetDate > today) {
      warnings.push('В календаре зал не запланирован. Черновик составляется по выбранному тренером методу; согласуйте рабочий день в ReadySix перед выполнением.');
      return choose('planning_only', 'Предварительный план по выбору тренера');
    }
...
export function readySixGenerationIssue(recommendation, focus) {
  ...
  if (recommendation.state.hardStop || (recommendation.state.capPercent != null && recommendation.state.capPercent <= 20) || (recommendation.state.needsReview && !recommendation.state.dataInsufficient)) return { status: recommendation.state.capPercent == null ? 503 : 409,
...
export function formatReadySixGymContext(recommendation) {
  ...
  return `\nREADYSIX — ИСТОЧНИК АНАЛИТИКИ И КАЛЕНДАРЯ:\n` +
    ...
    (recommendation.planning?.preliminary ? `ПРЕДВАРИТЕЛЬНЫЙ ПЛАН: тренировка ${recommendation.targetDate}, оценка состояния на ${recommendation.planning.assessmentDate}. Не требуй анкет из будущего. Не называй игрока готовым на будущую дату. Используй последнюю доступную историю, сохрани известные ограничения; новые анкеты проверяются перед выполнением.\n` : '') +
...
export function applyReadySixGymDose(prescription, recommendation) {
  ...
  // Consecutive matches are still match days. Yesterday's game must not
  // overwrite today's primer with an incompatible recovery prescription.
  const recoveryDay = recommendation.key !== 'planning_only' && (['rest', 'travel', 'recovery', 'volleyball_only'].includes(plan) || (!matchDay && calendar.daysSinceGame === 1));
  const pregame = !matchDay && (calendar.daysToGame === 1 || plan === 'pregame');
  ...
  next.readySix = { revision: recommendation.revision, planning: recommendation.planning || null, capPercent: factor * 100, calendarLimited: recoveryDay || pregame || matchDay };
  return next;
}
