import { auditExerciseVariety } from './exerciseVariety.mjs';
...
import { auditInSeasonStrengthSession, isStrengthMachineAccessoryAllowed } from './inSeasonStrength.mjs';
...
const STRENGTH_BENCH_RE = /barbell bench press|bench press barbell|жим штанги лёжа/i;
const STRENGTH_OHP_RE = /barbell overhead press|overhead press.*barbell|military press|жим штанги стоя/i;
const STRENGTH_MACHINE_PATTERNS = [
...
function recentExerciseContext(summaries = []) {
  ...
  recent.forEach((summary, summaryIndex) => {
    const matches = String(summary || '').matchAll(/\[([A-Z]\d+)\]\s+(.+?)\s+\(/gi);
...
export function validateSession(session, playerRestrictions = [], { seasonDecision = null, focus = '', dosePrescription = null } = {}) {
  ...
  const inSeasonStrength = !!dosePrescription?.strengthMode || String(focus) === 'inseason_strength';
    ...
    for (const [exerciseIndex, ex] of (block.exercises || []).entries()) {
      ...
      if (!approvedMatchDayExercise) {
        for (const re of FORBIDDEN_PATTERNS) {
          ...
          if (inSeasonStrength && [POWER_DEADLIFT_RE, STRENGTH_BENCH_RE].includes(re)) continue;
          if (inSeasonStrength && re === STRENGTH_OHP_RE && !dosePrescription?.strengthContext?.noOverhead) continue;
          if (inSeasonStrength && STRENGTH_MACHINE_PATTERNS.includes(re) && isStrengthMachineAccessoryAllowed(name, exerciseCode)) continue;
          if (re.test(name)) {
  ...
  const expectedStrengthPrehab = dosePrescription?.strengthMode === 'development' ? 'E'
    : dosePrescription?.strengthMode === 'maintenance' ? 'D' : null;
  const hasEBlock = session.blocks.some(b => {
    const label = String(b.code || b.label || '').trim().toUpperCase();
    return label.startsWith('E') || (expectedPowerPrehab && label.startsWith(expectedPowerPrehab))
      || (expectedStrengthPrehab && label.startsWith(expectedStrengthPrehab)) || label.toLowerCase().includes('проф');
  });
...
export function assessSessionQuality(session, {
  ...
  const strengthAudit = auditInSeasonStrengthSession(session, prescription);
  ...
  const seasonSafety = (() => {
    ...
    if (strengthAudit) {
      return {
        ok: strengthAudit.safe,
        detail: strengthAudit.safe
          ? `силовая методика безопасна: ${strengthAudit.detail}`
          : `нарушены лимиты силовой методики: ${strengthAudit.detail}`,
      };
    }
  ...
  const expectedBlocks = prescription.powerMode === 'development' ? [6, 6]
    : prescription.powerMode === 'microdose' ? [4, 4]
      : prescription.strengthMode === 'development' ? [5, 5]
        : prescription.strengthMode === 'maintenance' ? [4, 4]
  ...
  const variety = auditExerciseVariety(session, varietyContext);
  const variationOk = variety ? variety.ok : matchDay || accessoryOverlap <= 0.4;
  ...
  const checks = [
    ...
    ...(strengthAudit ? [{
      id: 'strength_method', label: 'Методика силы/поддержания', maxPoints: 0,
      ok: strengthAudit.valid,
      detail: strengthAudit.detail,
    }] : []),
    {
    ...
    },
    {
      ...
      ok: variationOk,
      detail: variety ? variety.detail : recent.lastTwo.size === 0 ? 'история для сравнения ещё не накоплена' : `${Math.round(accessoryOverlap * 100)}% аксессуаров повторяют последние 2 сессии`,
    },
    {
  ...
  const criticalChecks = ['safety', 'season_safety', 'power_method', 'strength_method', 'uniqueness', 'structure', 'dose', 'completeness'];
...
export function qualityCorrectionPrompt(userPrompt, session, quality) {
  ...
  return `${userPrompt}\n\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\nПРОФЕССИОНАЛЬНЫЙ КОНТРОЛЬ КАЧЕСТВА: ${quality?.score ?? 0}/100\n${issues || '• Нужна более точная и профессиональная версия программы.'}\n\nПредыдущая версия:\n${JSON.stringify(session)}\n\nИсправь только отмеченные недостатки, сохрани удачные решения и ручной метод тренера. Сохрани максимум два подходящих главных якоря прогрессии. Замени перечисленные недавние вспомогательные упражнения безопасными вариантами той же задачи; не маскируй повтор новым названием. Ограничения и дозировка имеют приоритет. Верни только улучшенную тренировку через build_session.\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`;
}
