...
export function validateSession(session, playerRestrictions = [], { seasonDecision = null } = {}) {
  const errors = [];
  const warnings = [];
...
export function assessSessionQuality(session, {
  ...
  const validation = validateSession(session, playerRestrictions, { seasonDecision });
  ...
  const expectedBlocks = matchDay ? [3, 3] : recovery || activation ? [3, 5] : [4, 5];
