...
export function formatPerformanceKpisForPrompt(neuro, targetDate) {
  ...
  lines.push('→ Протокол RSI Заречья: повторные вертикальные прыжки. Сравнивай только результаты одного и того же протокола.');
  ...
  lines.push('→ Протокол 10 м Заречья: электронные ворота, одна валидная попытка. Не смешивай с ручным хронометражем или другим стартовым протоколом.');
