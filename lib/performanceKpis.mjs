const METRICS = {
  ...
  attackJump: {
    label: 'Атакующий прыжок',
    unit: ' см',
    aliases: [
      'attackJump', 'attack_jump', 'approachJump', 'approach_jump',
      'spikeJump', 'spike_jump', 'aj',
    ],
    cadenceDays: 7,
    staleAfterDays: 8,
    higherIsBetter: true,
...
export function formatPerformanceKpisForPrompt(neuro, targetDate) {
  ...
  for (const key of ['rsi', 'cmj', 'attackJump', 'sprint10m']) {
  ...
  lines.push('→ Устаревший тест остаётся контекстом, но не считается свежим сигналом готовности. RSI ожидается ежедневно; CMJ, атакующий прыжок и 10 м — еженедельно.');
