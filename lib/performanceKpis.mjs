...
export function formatPerformanceKpisForPrompt(neuro, targetDate) {
  ...
  lines.push('→ Протокол CMJ Заречья: с работой рук, 3 валидные попытки, в систему записывается лучший результат. Сравнивай только тесты, выполненные по этому же протоколу.');
