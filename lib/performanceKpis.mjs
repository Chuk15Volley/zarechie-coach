...
function measurementNoise(series) {
  const window = series.filter(item => Number.isFinite(item.value)).slice(-8);
  if (window.length < 6) return { typicalError: null, typicalErrorPercent: null, decisionThresholdPercent: 5, calibrated: false };
  const diffs = window.slice(1).map((item, index) => item.value - window[index].value);
  const meanDiff = diffs.reduce((sum, value) => sum + value, 0) / diffs.length;
  const variance = diffs.reduce((sum, value) => sum + (value - meanDiff) ** 2, 0) / Math.max(1, diffs.length - 1);
  const typicalError = Math.sqrt(variance) / Math.sqrt(2);
  const mean = window.reduce((sum, item) => sum + item.value, 0) / window.length;
  const typicalErrorPercent = mean ? Math.abs(typicalError / mean) * 100 : null;
  // A change must exceed ordinary within-player noise. The 3–12% bounds avoid
  // unstable early estimates creating implausibly tiny or unusably large gates.
  const threshold = typicalErrorPercent == null ? 5 : Math.min(12, Math.max(3, typicalErrorPercent * 1.5));
  return {
    typicalError: round(typicalError, 2),
    typicalErrorPercent: round(typicalErrorPercent, 1),
    decisionThresholdPercent: round(threshold, 1),
    calibrated: true,
  };
}
...
export function performanceKpis(neuro, targetDate) {
  ...
  for (const [key, config] of Object.entries(METRICS)) {
    ...
    const noise = measurementNoise(series);
    const performanceDelta = rawDeltaPercent == null ? null : (config.higherIsBetter ? rawDeltaPercent : -rawDeltaPercent);
    ...
    result[key] = {
      ...
      performanceDeltaPercent: performanceDelta == null ? null : round(performanceDelta, 1),
      ...noise,
      meaningfulChange: performanceDelta == null ? false : Math.abs(performanceDelta) >= noise.decisionThresholdPercent,
      meaningfulDecline: performanceDelta == null ? false : performanceDelta <= -noise.decisionThresholdPercent,
      changeSignal: performanceDelta == null || Math.abs(performanceDelta) < noise.decisionThresholdPercent
        ? 'within_noise'
        : performanceDelta <= -noise.decisionThresholdPercent * 2 ? 'red'
        : performanceDelta < 0 ? 'yellow' : 'positive',
...
export function formatPerformanceKpisForPrompt(neuro, targetDate) {
  ...
  for (const key of ['rsi', 'cmj', 'sprint10m']) {
    ...
    const trend = metric.baseline != null
      ? `; индивидуальный baseline ${metric.baseline}${metric.unit}; изменение результативности ${metric.performanceDeltaPercent >= 0 ? '+' : ''}${metric.performanceDeltaPercent}%; порог значимого изменения ${metric.decisionThresholdPercent}%${metric.calibrated ? ' (по индивидуальному шуму)' : ' (временный до 6 тестов)'}`
  ...
  lines.push('→ Решение об урезании нагрузки принимай только если изменение превышает рассчитанный порог шума и совпадают минимум два домена (KPI + WHOOP/опрос/боль).');
