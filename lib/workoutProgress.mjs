import { validRepetitions } from './playerGym.mjs';
import { normalizeSkips, skippedSetCount } from './playerExperience.mjs';
import { FINISH_REASONS } from './playerWorkout.mjs';
...
export function mergeWorkoutProgress(existing = {}, incoming = {}, now = new Date().toISOString()) {
  ...
  const cleanReps = Object.fromEntries(Object.entries(safeObject(next.repetitions)).filter(([key, value]) => /^\d+-\d+-\d+$/.test(key) && (value === '' || validRepetitions(value) != null)));
  const repsMerge = mergeField(current.repetitions, current.repsUpdatedAt, cleanReps, next.repsUpdatedAt, fallbackClock);
  const skipMerge = mergeField(current.skipped, current.skipUpdatedAt, normalizeSkips(next.skipped), next.skipUpdatedAt, fallbackClock);
  ...
  return {
    ...current,
    skipped: skipMerge.values,
    skipUpdatedAt: skipMerge.clocks,
    done: doneMerge.values,
    weights: weightMerge.values,
    repetitions: repsMerge.values,
    repsUpdatedAt: repsMerge.clocks,
    ...
    finishReason: incomingMetadataIsNewer && Object.hasOwn(next, 'finishReason') ? (FINISH_REASONS.includes(next.finishReason) ? next.finishReason : null) : (current.finishReason || null),
    ...
    restPausedSeconds: incomingMetadataIsNewer && Object.hasOwn(next, 'restPausedSeconds') ? Math.max(0, Math.min(300, Number(next.restPausedSeconds) || 0)) : (current.restPausedSeconds || 0),
...
export function summarizePlayerWorkout(session, log = {}, feedback = null, now = new Date().toISOString()) {
  ...
  const blockStats = blocks.map((block, bi) => {
    ...
    const exercises = (block.exercises || []).map(exercise => ({
      ...
      code: exercise.code || '',
      name: exercise.name || '',
      weightKg: Number(exercise.weightKg) || null,
    }));
    return {
      label: block.label || String.fromCharCode(65 + bi),
      total,
      completed,
      complete: total > 0 && total === completed,
      exercises,
      station: equipmentCategory(exercises),
    };
  });
  ...
  const activeIndex = blockStats.findIndex((block, bi) => (blocks[bi].exercises || []).some((ex, ei) => !log?.skipped?.[`${bi}-${ei}`] && (ex.targetSets || []).some((_, si) => !done[`${bi}-${ei}-${si}`])));
  const skippedSets = skippedSetCount(session, done, log?.skipped);
  const completed = totalSets > 0 && completedSets === totalSets;
  const endedEarly = Boolean(log?.completedAt && (log?.finishReason || skippedSets > 0) && !completed);
  const ended = completed || endedEarly;
  const started = Boolean(log?.startedAt || completedSets > 0);
  const lastSyncMs = log?.savedAt ? new Date(log.savedAt).getTime() : 0;
  const stale = started && !ended && lastSyncMs > 0 && (new Date(now).getTime() - lastSyncMs) > 120000;
  const restOverdue = Boolean(log?.restUntil && !ended && new Date(log.restUntil).getTime() < new Date(now).getTime());
  ...
  return {
    ...
    endedEarly,
    skippedSets,
    skippedExercises: Object.entries(log?.skipped || {}).filter(([, reason]) => reason).map(([key, reason]) => { const [bi, ei] = key.split('-').map(Number); return { name: blocks[bi]?.exercises?.[ei]?.name || key, reason }; }),
    finishReason: log?.finishReason || null,
    totalSets,
    completedSets,
    progress: totalSets ? Math.round(completedSets / totalSets * 100) : 0,
    activeBlock: !ended && activeIndex >= 0 ? blockStats[activeIndex]?.label : null,
    blockStats,
...
import { equipmentCategory } from './floorOperations.mjs';
