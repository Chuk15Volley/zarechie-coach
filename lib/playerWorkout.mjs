...
export function blockIsComplete(block, bi, done = {}) {
  const exercises = Array.isArray(block?.exercises) ? block.exercises : [];
  const sets = exercises.flatMap((exercise, ei) =>
    (Array.isArray(exercise?.targetSets) ? exercise.targetSets : []).map((_, si) => `${bi}-${ei}-${si}`)
  );
  return sets.length > 0 && sets.every(key => Boolean(done[key]));
}

export function firstIncompleteBlock(session, done = {}) {
  const blocks = Array.isArray(session?.blocks) ? session.blocks : [];
  const bi = blocks.findIndex((block, index) => !blockIsComplete(block, index, done));
  return bi >= 0 ? { bi, block: blocks[bi] } : null;
}

export function nextIncompleteBlock(session, currentBi, done = {}) {
  const blocks = Array.isArray(session?.blocks) ? session.blocks : [];
  const after = blocks.findIndex((block, index) => index > currentBi && !blockIsComplete(block, index, done));
  if (after >= 0) return { bi: after, block: blocks[after] };
  const remaining = blocks.findIndex((block, index) => index !== currentBi && !blockIsComplete(block, index, done));
...
export function nextWorkoutSet(session, done = {}) {
  for (const [bi, block] of (session?.blocks || []).entries()) {
    ...
    const next = candidates.find(item => !done[item.key]);
    if (next) return next;
  }
  return null;
}
...
export function completedTonnage(session, done = {}, weights = {}) {
  ...
  for (const { bi, ei, exercise } of workoutExercises(session)) {
    ...
    for (const [si, target] of (exercise.targetSets || []).entries()) {
      ...
      tonnage += weight * units * actualRepsFromTarget(target);
    }
  }
