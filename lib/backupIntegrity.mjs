export function sessionInventory(snapshot) {
  ...
  const libraryIds = snapshot.workspace === 'zarechie' ? snapshot.entries.filter(e => /^ex:lib:[^:]+$/.test(e.key) && e.type === 'hash' && e.ttlMs !== -2).map(e => e.key.slice(7)).sort() : [];
  return { workspace: snapshot.workspace, backupId: snapshot.id, createdAt: snapshot.createdAt, players, libraryIds, sessionCount: Object.values(players).reduce((sum, dates) => sum + dates.length, 0) };
}
...
export function compareSessionInventory(baseline, current) {
  ...
  const library = new Set(current.libraryIds || []);
  for (const id of baseline.libraryIds || []) if (!library.has(id)) missing.push({ exerciseId: id });
