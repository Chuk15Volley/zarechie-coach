...
const RECORD_ROLLUP_SCRIPT = "local p=ARGV[1]..':' local g=ARGV[7] redis.call('hincrby',KEYS[1],p..'n',1) redis.call('hincrby',KEYS[1],p..'c',ARGV[2]) redis.call('hincrby',KEYS[1],p..'o',ARGV[3]) redis.call('hincrby',KEYS[1],p..'s',ARGV[4]) redis.call('hincrby',KEYS[1],p..'b'..ARGV[5],1) redis.call('hincrby',KEYS[1],p..g..'n',1) redis.call('hincrby',KEYS[1],p..g..'o',ARGV[8]) redis.call('hincrby',KEYS[1],p..g..'s',ARGV[4]) redis.call('hincrby',KEYS[1],p..g..'b'..ARGV[5],1) redis.call('expire',KEYS[1],ARGV[6]) return 1";

function isWarmCache(cache) {
  return ['hit', 'stale', 'stale-while-revalidate'].includes(cache);
}
...
export async function recordReadinessLatency({ workspace = 'zarechie', durationMs, cache, playerCount, date, environment, release, now = Date.now(), redisPipelineImpl = redisPipeline }) {
  ...
  const warm = isWarmCache(sample.cache);
  const segment = warm ? 'w' : 'x';
  const segmentTarget = warm ? WARM_TARGET_MS : COLD_TARGET_MS;
  await redisPipelineImpl([
    ['LPUSH', key, JSON.stringify(sample)],
    ['LTRIM', key, '0', String(SAMPLE_LIMIT - 1)],
    ['EVAL', RECORD_ROLLUP_SCRIPT, '1', rollupKey, String(minute), warm ? '1' : '0', sample.durationMs > TARGET_MS ? '1' : '0', String(sample.durationMs), String(histogramBucket(sample.durationMs)), String(ROLLUP_TTL_SECONDS), segment, sample.durationMs > segmentTarget ? '1' : '0'],
  ]).catch(() => {});
  return sample;
}
...
export function summarizeReadinessLatency(rows, options = {}) {
  ...
  const warm = summarizeRecentSegment(samples.filter(sample => isWarmCache(sample.cache)), WARM_TARGET_MS, MIN_HEALTH_SAMPLES);
  const cold = summarizeRecentSegment(samples.filter(sample => !isWarmCache(sample.cache)), COLD_TARGET_MS, MIN_COLD_SAMPLES);
  const segmented = warm.enoughSamples || cold.enoughSamples;
  return {
    ...
    enoughSamples,
    healthy: segmented ? warm.healthy && cold.healthy : !enoughSamples || p95Ms <= TARGET_MS,
    overallHealthy: !enoughSamples || p95Ms <= TARGET_MS,
    warm,
    cold,
...
function summarizeRecentSegment(samples, targetMs, minimumSamples) {
  const durations = samples.map(sample => Math.round(Number(sample.durationMs))).sort((a, b) => a - b);
  const p95Ms = percentile(durations, 0.95);
  const enoughSamples = durations.length >= minimumSamples;
  return {
    ...
    maxMs: durations.at(-1) ?? null,
    enoughSamples,
    healthy: !enoughSamples || p95Ms <= targetMs,
  };
}
...
export function summarizeReadinessRollups(hashes, options = {}) {
  ...
  const segments = {
    w: { sampleCount: 0, violationCount: 0, durationTotalMs: 0, histogram: Array(HISTOGRAM_BOUNDS_MS.length).fill(0) },
    x: { sampleCount: 0, violationCount: 0, durationTotalMs: 0, histogram: Array(HISTOGRAM_BOUNDS_MS.length).fill(0) },
  };

  for (const hash of Array.isArray(hashes) ? hashes : []) {
    for (const [rawField, rawValue] of hashEntries(hash)) {
      const match = String(rawField).match(/^(\d+):(n|c|o|s|b(\d+)|([wx])(n|o|s|b(\d+)))$/);
      ...
      const segmentMetric = match[5];
      if (segment && segmentMetric) {
        if (segmentMetric === 'n') segments[segment].sampleCount += value;
        else if (segmentMetric === 'o') segments[segment].violationCount += value;
        else if (segmentMetric === 's') segments[segment].durationTotalMs += value;
        else {
          const bucket = Number(match[6]);
          if (Number.isInteger(bucket) && bucket >= 0 && bucket < HISTOGRAM_BOUNDS_MS.length) segments[segment].histogram[bucket] += value;
        }
        continue;
      }
  ...
  const warm = summarizeRollupSegment(segments.w, WARM_TARGET_MS, MIN_HEALTH_SAMPLES);
  const cold = summarizeRollupSegment(segments.x, COLD_TARGET_MS, MIN_COLD_SAMPLES);
  const segmented = warm.enoughSamples || cold.enoughSamples;
  const overallHealthy = !enoughSamples || targetViolationCount <= Math.floor(sampleCount * 0.05);
  return {
    sampleCount,
    ...
    enoughSamples,
    healthy: segmented ? warm.healthy && cold.healthy : overallHealthy,
    overallHealthy,
    warm,
    cold,
...
function summarizeRollupSegment(segment, targetMs, minimumSamples) {
  const enoughSamples = segment.sampleCount >= minimumSamples;
  return {
    sampleCount: segment.sampleCount,
    targetMs,
    p50Ms: histogramPercentile(segment.histogram, segment.sampleCount, 0.5),
    p95Ms: histogramPercentile(segment.histogram, segment.sampleCount, 0.95),
    maxMs: histogramPercentile(segment.histogram, segment.sampleCount, 1),
    averageMs: segment.sampleCount ? Math.round(segment.durationTotalMs / segment.sampleCount) : null,
    violationCount: segment.violationCount,
    enoughSamples,
    healthy: !enoughSamples || segment.violationCount <= Math.floor(segment.sampleCount * 0.05),
  };
}
...
export const READINESS_WARM_TARGET_MS = WARM_TARGET_MS;
export const READINESS_COLD_TARGET_MS = COLD_TARGET_MS;
