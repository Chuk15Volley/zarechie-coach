...
export function tempoDescription(tempo, exerciseName = '', targetSets = []) {
  const value = clean(tempo);
  const kind = movementKind(exerciseName);
  const timedTargets = Array.isArray(targetSets) && targetSets.length > 0
    && targetSets.every(target => /^\d+(?:[.,]\d+)?(?:\s*[-–]\s*\d+)?\s*(?:сек(?:унд[а-я]*)?|sec(?:onds?)?|s|с)(?=$|[\s/])/i.test(clean(target)));
  if (timedTargets && (kind === 'isometric' || /spanish squat|испанск.*присед/i.test(exerciseName) || /изометр|\biso\b|удерж/i.test(value))) {
    const targets = [...new Set(targetSets.map(clean))];
    const hold = targets.length === 1 ? targets[0] : 'время, указанное в каждом подходе';
...
function generatedExerciseDescription(exercise = {}) {
  ...
  if (['upper', 'lower', 'unloaded'].includes(exercise.prehabRegion)) {
    const tempo = exercise.prehabRegion === 'unloaded' ? 'Темп: спокойное дыхание без задержек и усилия.' : 'Темп: плавно и под контролем в обе стороны; без рывка и взрывного усилия.';
    return `${tempo}${technique ? ` ${technique}` : ''}`;
  }
  return `${tempoDescription(exercise.tempo, exercise.name, exercise.targetSets)}${technique ? ` ${technique}` : ''}`.trim();
}
