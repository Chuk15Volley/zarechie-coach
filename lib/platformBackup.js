...
async function enforceRetention(workspace) {
  const listed = await list({ prefix: `${BACKUP_PREFIX}/${workspace}/`, limit: 100 });
...
export async function listEncryptedBackups(requestedWorkspace, limit = 14) {
  requireConfiguration();
  const workspace = normalizeWorkspace(requestedWorkspace);
  const listed = await list({ prefix: `${BACKUP_PREFIX}/${workspace}/`, limit: Math.min(100, Math.max(1, Number(limit) || 14)) });
  return listed.blobs
...
export function backupIsConfigured() {
  return Boolean(process.env.BLOB_READ_WRITE_TOKEN && Buffer.byteLength(String(process.env.BACKUP_ENCRYPTION_KEY || '')) >= 32);
}
