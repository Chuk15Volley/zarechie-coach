...
export async function createEncryptedBackup(requestedWorkspace) {
  ...
  if (workspace === 'zarechie') {
    const libraryIndex = new Set(snapshot.entries.find(e => e.key === 'ex:index' && e.type === 'set')?.value || []);
    unindexed.push(...inventory.libraryIds.filter(id => !libraryIndex.has(id)));
    if (!Array.isArray(baseline?.libraryIds)) { integrity.ok = false; integrity.reason = 'library_baseline_missing'; }
  }
...
export async function checkHistoryIntegrity(workspace) {
  ...
  const current = { workspace, players: Object.fromEntries(ids.map((id, index) => [id, dates[index] || []])), libraryIds: workspace === 'zarechie' ? await redis('smembers', 'ex:index') : [] };
  ...
  keys.push(...(baseline.libraryIds || []).map(id => `ex:lib:${id}`));
  const commands = [];
  ...
  const libraryBaselineMissing = workspace === 'zarechie' && !Array.isArray(baseline.libraryIds);
  return { ok: result.ok && missingRecords === 0 && !libraryBaselineMissing, missingCount: result.missingCount, missingRecords, baselineSessions: baseline.sessionCount, baselineExercises: baseline.libraryIds?.length ?? null, libraryBaselineMissing, baselineCreatedAt: baseline.createdAt };
}
