...
export function buildDosePrescription({ focus = '', trainingType = '', coachRecovery = 'green' } = {}) {
  ...
  const exercises = coachRecovery === 'red'
    ? range(Math.max(4, Math.round(base.exercises.min * 0.6)), Math.max(5, Math.min(7, Math.round(base.exercises.max * 0.6))))
    : range(Math.max(5, Math.round(base.exercises.min * 0.75)), Math.max(7, Math.round(base.exercises.max * 0.8)));
  const minutes = coachRecovery === 'red'
    ? range(Math.max(25, Math.round(base.minutes.min * 0.6)), Math.max(35, Math.round(base.minutes.max * 0.72)))
    : range(Math.max(30, Math.round(base.minutes.min * 0.75)), Math.max(40, Math.round(base.minutes.max * 0.86)));
...
export function auditDose(session, prescription) {
  ...
  const safeChecks = {
    ...
    hardSets: actual.hardSets <= p.hardSets.max,
  ...
  const minimumViableChecks = {
    ...
    hardSets: p.hardSets.min === 0 || actual.hardSets >= Math.floor(p.hardSets.min * 0.5),
