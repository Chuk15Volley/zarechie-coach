...
const JUMP_RE = /jump|hop|pogo|bound|plyo|depth landing|drop landing|прыж|плиом|соскок/i;
...
export function buildDosePrescription({ focus = '', trainingType = '', coachRecovery = 'green', seasonContext = null } = {}) {
  ...
  if (seasonContext?.key === 'md_plus_1') {
    ...
    base = { key: 'match-day-primer', minutes: range(10, 18), exercises: range(3, 6), totalSets: range(5, 12), hardSets: range(0, 4), jumpContacts: range(2, 8), targetRpe: range(4, 6) };
  }
...
export function auditDose(session, prescription) {
  ...
  const minimumViableChecks = {
    ...
    duration: actual.estimatedMinutes >= Math.max(15, Math.floor(p.minutes.min * 0.6)),
  };
  const details = [
