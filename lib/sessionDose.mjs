...
const BALLISTIC_RE = /jump|hop|pogo|bound|plyo|throw|toss|chest pass|slam|depth landing|drop landing|прыж|плиом|брос/i;
...
function profileFor(focus = '', trainingType = '') {
  ...
  if (/inseason_power|conversion/.test(f)) {
    return { key: 'in-season-power', minutes: range(25, 40), exercises: range(5, 8), totalSets: range(10, 22), hardSets: range(2, 10), jumpContacts: range(6, 18), targetRpe: range(5, 7) };
  }
...
export function buildDosePrescription({ focus = '', trainingType = '', coachRecovery = 'green', seasonContext = null, matchDayPrimer = null } = {}) {
