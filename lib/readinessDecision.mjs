import { readySixGymState } from './readySixGym.mjs';
...
export function readinessDecisionLevel({ evening, eveningFresh, postMorning, postMorningFresh, morning, morningFresh, whoop, neuro, activeInjuries, testsExpected = true }) {
  ...
  if (activeInjuries.length || freshLoadConcern || freshStrongestPain >= 3) {
    return { code: 'injury_or_pain', level: 'red', label: 'Нужна адаптация', detail: 'Есть свежая травма или выраженная боль.' };
  }
  if (anyLoadConcern || strongestPain >= 3) {
    return { code: 'previous_injury_or_pain', level: 'yellow', label: 'Требует проверки', detail: 'В последней, но не свежей анкете есть боль или травма: проверь актуальность перед стартом.' };
  }
  if (unscoredFreshPain) {
    return { code: 'unscored_pain', level: 'yellow', label: 'Требует проверки', detail: 'В свежей анкете отмечена боль без уровня: уточни её перед стартом и не форсируй нагрузку на эту зону.' };
  }
  if (readinessNumber(whoop?.recovery) != null && readinessNumber(whoop.recovery) < 34) {
    return { code: 'recovery_low', level: 'red', label: 'Только качество', detail: 'Recovery ниже 34%: снизить объём и риск.' };
  }
  if (readinessNumber(morning?.readiness) != null && readinessNumber(morning.readiness) <= 2) {
    return { code: 'morning_readiness_low', level: 'red', label: 'Только качество', detail: 'Игрок отметил низкую утреннюю готовность.' };
  }
  if (sessionSurveys.some(item => readinessNumber(item.record?.tomorrowReadiness) != null && readinessNumber(item.record.tomorrowReadiness) <= 2)) {
    return { code: 'next_day_readiness_low', level: 'yellow', label: 'Объём снижен', detail: 'Низкая готовность по последней анкете после нагрузки.' };
  }
  if (sessionSurveys.some(item => readinessNumber(item.record?.soreness) >= 4 || readinessNumber(item.record?.fatigue) >= 4 || readinessNumber(item.record?.legFatigue) >= 4 || readinessNumber(item.record?.shoulderLoad) >= 4)) {
    return { code: 'local_fatigue_high', level: 'yellow', label: 'Нужна коррекция', detail: 'Высокая локальная усталость или крепатура.' };
  }
  ...
  if (availableDomains < 2) {
    return {
      code: 'insufficient_data', level: 'yellow',
  ...
  return { code: 'clear', level: 'green', label: 'Данные без красных флагов', detail: 'Тренерский статус и выбранная тема остаются решающими.' };
}

export function readySixDecisionLevel(decision) {
  if (!decision || typeof decision !== 'object') return null;
  const strengthTargets = (decision.targets || []).filter(target =>
    target?.target === 'strength_upper' || target?.target === 'strength_lower'
  );
  const relevant = strengthTargets.length ? strengthTargets : [decision];
  const caps = relevant.map(item => readinessNumber(item?.capPercent)).filter(value => value != null);
  const capPercent = caps.length ? Math.min(...caps) : readinessNumber(decision.capPercent);
  const hardStopSignal = decision.hardStopSignal === true || relevant.some(item => item?.hardStopSignal === true);
  const reasons = [...new Set(relevant.flatMap(item => item?.reasons || []).concat(decision.reasons || []))].slice(0, 3);
  const restrictions = [...new Set(relevant.flatMap(item => item?.restrictions || []).concat(decision.restrictions || []))].slice(0, 3);
  const detailParts = [
    capPercent != null ? `Лимит силовой нагрузки ReadySix: ${capPercent}%.` : null,
    reasons.length ? `Причины: ${reasons.join('; ')}.` : null,
    restrictions.length ? `Ограничения: ${restrictions.join('; ')}.` : null,
  ].filter(Boolean);

  if (hardStopSignal || (capPercent != null && capPercent <= 20)) {
    return { code: 'readysix_hard_limit', level: 'red', label: 'ReadySix: нужна адаптация', detail: detailParts.join(' ') };
  }
  if (
    ...
    return { code: 'readysix_insufficient_data', level: 'yellow', label: 'ReadySix: данных недостаточно', detail: detailParts.join(' ') || 'Не считать отсутствие данных зелёным сигналом.' };
  }
  if (capPercent < 100) {
    return { code: 'readysix_modified', level: 'yellow', label: 'ReadySix: нагрузка ограничена', detail: detailParts.join(' ') };
  }
  return { code: 'readysix_clear', level: 'green', label: 'ReadySix: ограничений нет', detail: detailParts.join(' ') || 'Значимых ограничений силовой нагрузки не выявлено.' };
}
...
export function readinessDecisionFromSnapshot(snapshot, targetDate, { testsExpected = true, neuroFresh = false } = {}) {
  ...
  const localDecision = readinessDecisionLevel({
    evening,
    eveningFresh,
    postMorning,
    postMorningFresh,
  ...
  const readySixDecision = readySixDecisionLevel(snapshot?.readySixDecision);
  const rank = { green: 0, yellow: 1, red: 2 };
  const decision = snapshot?.readySixMeta ? readySixGymState(snapshot.readySixDecision) : readySixDecision && rank[readySixDecision.level] > rank[localDecision.level]
    ? readySixDecision
    : localDecision;
  return { evening, postMorning, exactMorning, morning, whoop, eveningFresh, postMorningFresh, morningFresh, activeInjuries, decision, readySixDecision };
}
