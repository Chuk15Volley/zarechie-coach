import { approvedProgramLabel, approvedProgramText } from './programApproval.mjs';
...
export function parseSavedSession(value) {
  ...
  let session = Array.isArray(record.session?.blocks)
    ? record.session
    : Array.isArray(record.blocks)
      ? record
      : null;
  if (session) {
    session = { ...session, ...Object.fromEntries(['warnings', 'assessment', 'periodization_note'].filter(key => typeof session[key] === 'string').map(key => [key, approvedProgramText(session[key])])) };
    record = record.session ? { ...record, session, trainingLabel: approvedProgramLabel(record.trainingLabel) } : session;
  }
...
export function sessionTrainingLabel(value) {
  ...
  const explicit = approvedProgramLabel(cleanText(record.trainingLabel || session?.trainingLabel || session?.title || session?.name));
