...
const config = { configured: true, url: 'https://alerts.example.test/hook', secret, reason: null };
...
test('webhook configuration requires credential-free HTTPS and a strong signing secret', () => {
  assert.equal(operationalAlertConfiguration({ ALERT_WEBHOOK_URL: 'http://example.test', ALERT_WEBHOOK_SECRET: secret }).configured, false);
  assert.equal(operationalAlertConfiguration({ ALERT_WEBHOOK_URL: 'https://user:pass@example.test', ALERT_WEBHOOK_SECRET: secret }).configured, false);
  assert.equal(operationalAlertConfiguration({ ALERT_WEBHOOK_URL: 'https://example.test', ALERT_WEBHOOK_SECRET: 'short' }).configured, false);
  assert.equal(operationalAlertConfiguration({ ALERT_WEBHOOK_URL: 'https://example.test/hook', ALERT_WEBHOOK_SECRET: secret }).configured, true);
});
...
test('Vercel log fallback is stateful and independently deduplicated', async () => {
  let saved;
  const fallback = { configured: false, url: null, secret: '', reason: 'not configured' };
  ...
  assert.equal(first.status, 'log_firing');
...
test('SLO evaluator detects degraded p95 and repeated ReadySix failures', async () => {
  ...
  assert.deepEqual(result.activeConditions.sort(), ['readiness_p95', 'readysix_errors']);
  assert.equal(result.readySixErrors10m, 3);
  assert.equal(dispatched.length, 2);
  assert.equal(dispatched.every(condition => condition.active), true);
});
...
test('SLO cron is authenticated, workspace-aware and runs every five minutes', () => {
  ...
  assert.match(dashboard, /webhook подключён/);
