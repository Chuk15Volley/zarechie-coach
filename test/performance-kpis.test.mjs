...
test('selects the latest available result independently for every KPI', () => {
  const neuro = {
    history: [
      ...
      { date: '2026-07-27', cmj: 42, sprint: 1.85, attack_jump: 55 },
      { date: '2026-08-03', cmj: 38, sprint: 1.95, attack_jump: 57 },
  ...
  assert.deepEqual(
    {
      ...
      attackJump: [result.attackJump.value, result.attackJump.date],
      sprint10m: [result.sprint10m.value, result.sprint10m.date],
    },
    {
      rsi: [2.2, '2026-08-06'],
      cmj: [38, '2026-08-03'],
      attackJump: [57, '2026-08-03'],
...
test('reads nested dashboard history and attack-jump aliases', () => {
  const neuro = {
    latest: {
      hist: {
        rsi: [{ date: '2026-08-05', value: '1.91' }],
        cmj: [{ date: '2026-08-01', height: 41.2 }],
        approach_jump: [{ date: '2026-08-02', value: 59 }],
  ...
  assert.equal(result.attackJump.value, 59);
...
test('does not leak a generic legacy CMJ history into the other KPIs', () => {
  ...
  assert.equal(result.attackJump.value, null);
...
test('prompt exposes dates, baselines and missing KPI data', () => {
  ...
  assert.match(text, /Атакующий прыжок: данных нет/);
